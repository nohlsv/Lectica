<template>
    ...
    <AppLayout>
        <div class="bg-gradient min-h-screen space-y-6 p-6 sm:px-6 lg:px-8">
            <div class="mx-auto mb-6 flex max-w-lg justify-center sm:order-2 sm:flex-1">
            ...
            <div class="mx-full bg-container my-2 overflow-hidden shadow-sm sm:rounded-lg dark:bg-gray-800">
                <div class="gap-2 p-6">
                    <div class="-mx-5.5 flex flex-col bg-black/60 px-4 py-4">
                        <div class="flex items-center gap-4">
                            <h1 class="pixel-outline text-xl font-bold text-gray-100">{{ collection.name }}</h1>
                            <span
                                v-if="!collection.is_public"
                                class="pixel-outline inline-flex items-center rounded-full bg-gray-700 px-2 py-1 text-xs font-medium text-gray-300"
                            >
                                Private
                            </span>
                            <span
                                v-else
                                class="pixel-outline inline-flex items-center rounded-full bg-green-900 px-2 py-1 text-xs font-medium text-green-300"
                            >
                                Public
                            </span>
                            <span
                                v-if="!collection.is_original"
                                class="pixel-outline inline-flex items-center rounded-full bg-blue-900 px-2 py-1 text-xs font-medium text-blue-300"
                            >
                                Copy
                            </span>
                        </div>
                    ...
                    <div class="pixel-outline mt-2 flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        >
                            <svg class="pixel-outline-icon mr-2 h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                                <path
            ...
            <div class="py-6">
                <div class="mx-full sm:-mx-5.5 sm:px-6 lg:px-8">
                    <!-- Files in Collection -->
                    <div class="-mt-8 overflow-hidden rounded-md bg-black/60 shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <div class="mb-6 flex items-center justify-between">
                                <h3 class="text-md pixel-outline animate-soft-bounce font-medium text-yellow-500 sm:text-lg">
                            ...
                            <div v-if="collection.files.length > 0" class="space-y-4">
                                ...
                                >
                                    <div class="flex flex-col items-start space-y-2 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
                                        <div class="pixel-outline-icon flex-shrink-0">
                                            <span
                                        ...
                                        <div class="flex-1">
                                            <h4 class="pixel-outline text-sm font-medium text-gray-100 sm:mt-0">
                                                {{ file.title || file.name }}
                                            </h4>
                                            <p v-if="file.description" class="pixel-outline text-sm text-gray-400">
                                                {{ file.description }}
                                            </p>
                                            <div
                                                class="pixel-outline mt-1 flex gap-3 space-y-1 text-xs text-gray-400 sm:flex-row sm:space-y-0 sm:space-x-4"
                                            >
                                    ...
                                    <div class="mt-1 flex items-center space-x-2">
                                        <Link
                                            ...
                                            class="pixel-outline text-sm text-indigo-600 hover:text-indigo-900 hover:underline dark:text-indigo-500"
                                        >
                                            View
                                        </Link>
                                        <button
                                            ...
                                            class="pixel-outline text-sm text-red-600 hover:text-red-900 hover:underline dark:text-red-400"
                                        >
                                            Remove
                                        </button>
                                    </div>
                            ...
                            <div v-else class="py-12 text-center">
                                ...
                                <h3 class="pixel-outline mt-2 text-sm font-medium text-gray-100">No files in collection</h3>
                                <p class="pixel-outline mt-1 text-sm text-gray-400">This collection doesn't contain any files yet.</p>
