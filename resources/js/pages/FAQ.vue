...
import { BookOpen, ChevronDown, FileText, Settings, Trophy, Users, Zap } from 'lucide-vue-next';
import { ref } from 'vue';
        ...
        <div class="bg-lectica min-h-screen">
            <div class="container mx-auto px-4 py-8">
                ...
                <div class="mb-12 text-center">
                    <h1 class="pixel-outline mb-4 text-4xl font-bold text-white">Frequently Asked Questions</h1>
                    <p class="mx-auto max-w-3xl text-xl text-white/90">
                        Everything you need to know to get started with Lectica. From basic usage to advanced features and gamification mechanics.
                    </p>
                </div>
                ...
                <div class="mb-8">
                    <div class="flex flex-wrap justify-center gap-4">
                        <button
                            ...
                            class="bg-container pixel-outline flex items-center gap-2 rounded-lg px-4 py-2 text-white transition-colors hover:bg-white/20"
                        >
                ...
                <div class="mx-auto max-w-4xl space-y-6">
                    <Card v-for="section in faqSections" :key="section.id" class="bg-container border-gold/30 border-2 shadow-lg">
                        <Collapsible :open="openSections.includes(section.id)">
                            <CollapsibleTrigger @click="toggleSection(section.id)" class="w-full">
                                <CardHeader class="rounded-t-lg transition-colors hover:bg-white/10">
                                    <CardTitle class="pixel-outline flex items-center justify-between text-white">
                                        <div class="flex items-center gap-3">
                                            <component :is="section.icon" class="text-gold h-6 w-6" />
                                            <span class="text-xl">{{ section.title }}</span>
                                        </div>
                                        <ChevronDown
                                            class="text-gold h-5 w-5 transition-transform duration-200"
                            ...
                            <CollapsibleContent>
                                <CardContent class="pt-0 pb-6">
                                    <div class="space-y-6">
                                        <div v-for="(item, index) in section.questions" :key="index" class="border-gold/50 border-l-4 py-2 pl-4">
                                            <h4 class="pixel-outline mb-2 font-semibold text-white">
                                                {{ item.question }}
                                            </h4>
